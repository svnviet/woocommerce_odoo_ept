...
#             partner=partner_obj.search([('name','=',name),('city','=',city),('street','=',address1),('street2','=',address2),('zip','=',zip),('country_id','=',country.id)],limit=1)
        ...
        if partner:
            ...
            instance.woo_property_account_payable_id and partner.write({'property_account_payable_id': instance.woo_property_account_payable_id.id})
            instance.woo_property_account_receivable_id and partner.write({'property_account_receivable_id': instance.woo_property_account_receivable_id.id})
    ...
    def get_woo_order_vals(self,result,workflow,invoice_address,instance,partner,shipping_address,pricelist_id,fiscal_position,payment_term,payment_gateway):
            ...
            if instance.woo_version == 'old':
                ...
                created_at = result.get("date_created_gmt").replace('T', ' ')
    ...
    def auto_import_woo_sale_order_ept(self,ctx={}):
        ...
        if woo_instance_id:
            ...
            if instance and instance.woo_version == 'old':
                ...
                self.import_new_woo_orders(instance,after_date,before_date,is_cron=True)
        return True
            ...
            for order in order_ids:  
                ...
                for line in order.get('shipping_lines',[]):
                    if shipping_taxable and float(order.get('shipping_tax')) > 0.0:                        
                        shipping_tax_ids =  self.get_woo_tax_id_ept(instance,tax_datas,tax_included)
                    else:
                        shipping_tax_ids = []
                    ...
                    if tax_included:
                        price = line.get('total',0) + line.get('total_tax',0)
                    else:
                        price = line.get('total')
                    line=self.create_woo_sale_order_line(line,shipping_tax_ids,shipping_product,1,fiscal_position,partner,pricelist_id,shipping_product and shipping_product.name or line.get('method_title'),sale_order,price,True)
    ...
    @api.model
    def import_new_woo_orders(self,instance=False,after_date=False,before_date=False,is_cron=True):
        instances=[]
        transaction_log_obj=self.env["woo.transaction.log"]
        if not instance:
            instances=self.env['woo.instance.ept'].search([('order_auto_import','=',True),('state','=','confirmed')])
        ...
        for instance in instances:
            ...
            if instance.store_timezone:
                after_date = pytz.utc.localize(datetime.strptime(after_date,
                '%Y-%m-%d %H:%M:%S')).astimezone(pytz.timezone(instance.store_timezone)).\
                    strftime('%Y-%m-%d %H:%M:%S') if after_date else False
                before_date = pytz.utc.localize(datetime.strptime(before_date,
                '%Y-%m-%d %H:%M:%S')).astimezone(pytz.timezone(instance.store_timezone)).\
...
#                 sale_order.onchange_partner_shipping_id()
                        ...
                        for tax_data in tax_datas:
                            ...
                            account_tax = self.env['account.tax'].search(
                                    [('price_include', '=', tax_included),
                                     ...
                                     ('company_id', '=', instance.warehouse_id.company_id.id)],
                                    limit=1)
                            account_tax and shipping_tax_ids.append(account_tax.id)
                    product=False    
                    delivery_method=line.get('method_title')
                    if delivery_method:
                        carrier=self.env['delivery.carrier'].search([('woo_code','=',delivery_method)],limit=1)
                        ...
                        if carrier.product_id:
                            shipping_product=carrier.product_id
                    if tax_included:
                        price = float(line.get('total',0)) + float(line.get('total_tax',0))
                    else:
                        price = float(line.get('total')) 
                    line=self.create_woo_sale_order_line(line,[(6,0,shipping_tax_ids)],shipping_product,1,fiscal_position,partner,sale_order.pricelist_id.id,shipping_product and shipping_product.name or line.get('method_title'),sale_order,price,True)
                        ...
                        for tax_data in tax_datas:
                            ...
                            account_tax = self.env['account.tax'].search(
                                    [('price_include', '=', tax_included),
                                     ...
                                     ('company_id', '=', instance.warehouse_id.company_id.id)],
                                    limit=1)
                            account_tax and fee_line_tax_ids.append(account_tax.id)
                    # fee_line_tax_ids =  self.get_woo_tax_id_ept(instance,tax_datas,False)
                    if fee_value:
                        self.create_woo_sale_order_line({},[(6,0,fee_line_tax_ids)],instance.fee_line_id,1,fiscal_position,partner,pricelist_id,fee,sale_order,fee_value,False)
            if import_order_ids:
                self.env['sale.workflow.process.ept'].auto_workflow_process(ids=import_order_ids)
                odoo_orders=self.browse(import_order_ids)
                for order in odoo_orders:
    ...
    def update_woo_order_status(self,instance):
        ...
        for instance in instances:
            wcapi = instance.connect_in_woo()
            sales_orders = self.search([('warehouse_id','=',instance.warehouse_id.id),('woo_order_id','!=',False),('woo_instance_id','=',instance.id)],order='date_order')
