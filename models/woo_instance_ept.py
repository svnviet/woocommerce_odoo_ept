...
class woo_instance_ept(models.Model):
    ...
    def _default_stock_field(self):
        stock_field = self.env['ir.model.fields'].search([('model_id.model','=','product.product'),('name','=','virtual_available')],limit=1)
    ...
    def _get_default_warehouse(self):
        warehouse = self.env['stock.warehouse'].search([('company_id','=',self.env.user.company_id.id)],limit=1,order='id')
    ...
    def _woo_tz_get(self):
        ...
        @author: Dipak Gogiya on Date 08-Jan-2020.
    ...
    import_order_status_ids = fields.Many2many('import.order.status','woo_instance_order_status_rel','instance_id','status_id',"Import Order Status",default=_default_order_status,help="Selected status orders will be imported from WooCommerce")
    ...
    discount_product_id=fields.Many2one("product.product","Discount",domain=[('type','=','service')],default=_default_discount_product)
    fee_line_id=fields.Many2one("product.product","Fees",domain=[('type','=','service')], default=_default_fee_product)
    ...
    sync_images_with_product=fields.Boolean("Sync Images?",help="Check if you want to import images along with products",default=True)
    ...
    currency_id = fields.Many2one("res.currency",string="Currency",help="Woo Commerce Currency.") # Currency Field Added By : Ajay Ghimre on 5 Nov 2018.
    auto_active_currency = fields.Boolean("Auto Active Currency",default=True,help="Automatically changes currency state to active if it is inactive.") # Auto Active Currency Field Added By : Ajay Ghimre on 5 Nov 2018.
    ...
    def get_woo_currency(self):
        ...
        if response.get('settings',False):
            ...
            if not currency:
                raise Warning("Currency {} not found in odoo.\nPlease make sure currency record is created for {} and is in active state.".format(currency_code,currency_code))
            return currency
    ...
    def sync_system_status(self,transaction_log_obj):
        ...
        if not isinstance(res,requests.models.Response):
            transaction_log_obj.create({'message': "Import Woo System Status \nResponse is not in proper format :: %s"%(res),
                                         'mismatch_details':True,
                                         'type':'system_status',
                                         'woo_self_id':self.id
                                        })
        ...
        try:
            ...
            transaction_log_obj.create({'message':"Json Error : While import system status from WooCommerce for self %s. \n%s"%(self.name,e),
                         'mismatch_details':True,
                         'type':'system_status',
                         'woo_self_id':self.id
                        })
