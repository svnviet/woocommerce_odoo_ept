...
class woo_process_import_export(models.TransientModel):
    ...
    instance_ids = fields.Many2many("woo.instance.ept", 'woo_instance_import_export_rel', 'process_id',
                                    'woo_instance_id', "Instances")
    ...
    is_unpublish_products = fields.Boolean("UnPublish Products",
                                           help="Unpublish all products that are exported and published.")

    is_update_products = fields.Boolean("Update Products",
                                        help="Update product details of products that are already exported.")
    ...
    is_update_order_status = fields.Boolean("Update Order Status",
                                            help="Update order status in WooCommerce if it is changed in Odoo.")

    is_export_product_tags = fields.Boolean("Export Product Tags",
                                            help="Export newly created product tags to WooCommerce.")
    is_update_product_tags = fields.Boolean("Update Product Tags")

    is_export_product_categ = fields.Boolean("Export Product Category",
                                             help="Export newly created product categories to WooCommerce.")
    ...
    sync_attributes = fields.Boolean("Sync Attributes", help="Import or Sync WooCommerce Atrributes and its Terms.",
                                     default=False)
    ...
    def default_get(self, fields):
        ...
        if 'default_instance_id' in self._context:
            ...
            if woo_instance:
                res.update({'past_orders_before_date': past_orders_before_date,
                            'past_orders_after_date': past_orders_after_date})
        ...
        for instance in instances:
            if woo_tmpl_ids:
                woo_templates = woo_product_tmpl_obj.search(
                    [('woo_instance_id', '=', instance.id), ('id', 'in', woo_tmpl_ids), ('exported_in_woo', '=', True)])
            else:
                woo_templates = woo_product_tmpl_obj.search(
    ...
    def export_product_tags(self):
        ...
        for instance in instances:
            ...
            if woo_tag_ids:
                woo_product_tags = product_tag_obj.search(
                    [('woo_instance_id', '=', instance.id), ('id', 'in', woo_tag_ids), ('exported_in_woo', '=', False)])
            else:
                woo_product_tags = product_tag_obj.search(
    ...
    def export_coupons(self):
        ...
        for instance in instances:
            ...
            if woo_coupons_code:
                woo_coupons = woo_coupons_obj.search(
                    [('woo_instance_id', '=', instance.id), ('id', 'in', woo_coupons_code)])
            else:
    ...
    def update_product_tags(self):
        ...
        for instance in instances:
            ...
            if woo_tag_ids:
                woo_product_tags = product_tag_obj.search(
                    [('woo_tag_id', '!=', False), ('woo_instance_id', '=', instance.id), ('id', 'in', woo_tag_ids),
                     ('exported_in_woo', '=', True)])
            else:
                woo_product_tags = product_tag_obj.search(
                    [('woo_tag_id', '!=', False), ('woo_instance_id', '=', instance.id),
                     ('exported_in_woo', '=', True)])
    ...
    def import_sale_orders(self):
        ...
        for instance in self.instance_ids:
            if not instance.import_order_status_ids:
                raise Warning('Please select atleast one Order Status in Settings to import orders for Instance %s' % (
                    instance.name))
            if instance.woo_version == 'old':
                ...
                before_date = self.past_orders_before_date
                after_date = self.past_orders_after_date
                sale_order_obj.import_new_woo_orders(instance, after_date, before_date, is_cron=False)
        return True
            ...
            for odoo_template in odoo_templates:
                ...
                for variant in odoo_template.product_variant_ids:
                    ...
                    woo_variant = woo_product_obj.search(
                        [('woo_instance_id', '=', instance.id), ('product_id', '=', variant.id)])
                    if not woo_variant:
                        woo_variant.create({'woo_instance_id': instance.id, 'product_id': variant.id,
                                            'woo_template_id': woo_template.id, 'default_code': variant.barcode,
                                            'name': variant.display_name, 'woo_variant_url': variant.image_url or ''})
        return True
        ...
        if categ_id:
            ...
            for categ_id in list(OrderedDict.fromkeys(reversed(ctg_list))):
                woo_product_parent_categ = categ_id.parent_id and woo_product_categ.search(
                    [('name', '=', categ_id.parent_id.name), ('woo_instance_id', '=', instance.id)], limit=1) or False
                if woo_product_parent_categ:
                    woo_product_category = woo_product_categ.search(
                        [('name', '=', categ_id.name), ('parent_id', '=', woo_product_parent_categ.id),
                         ('woo_instance_id', '=', instance.id)], limit=1)
                else:
                    woo_product_category = woo_product_categ.search(
                        [('name', '=', categ_id.name), ('woo_instance_id', '=', instance.id)], limit=1)
                if not woo_product_category:
                    if not categ_id.parent_id:
                        ...
                        parent_id = woo_product_categ.search(
                            [('name', '=', categ_id.parent_id.name), ('woo_instance_id', '=', instance.id)], limit=1)
                        woo_product_categ.create(
                            {'name': categ_id.name, 'woo_instance_id': instance.id, 'parent_id': parent_id.id})
                elif not woo_product_category.parent_id and categ_id.parent_id:
                    parent_id = woo_product_categ.search(
                        [('name', '=', categ_id.parent_id.name), ('parent_id', '=', woo_product_parent_categ.id),
    ...
    def publish_multiple_products(self):
        ...
        if self._context.get('process') == 'publish_multiple_products':
            ...
            woo_templates = woo_template_obj.search(
                [('id', 'in', woo_template_ids), ('exported_in_woo', '=', True), ('website_published', '=', False)])
        else:
    ...
    def unpublish_multiple_products(self):
        ...
        if self._context.get('process') == 'unpublish_multiple_products':
            woo_template_ids = self._context.get('active_ids', [])
            woo_templates = woo_template_obj.search(
                [('id', 'in', woo_template_ids), ('exported_in_woo', '=', True), ('website_published', '=', True)])
        else:
        ...
        for instance in instances:
            ...
            if instance.woo_version == 'old' and woo_templates:
                woo_product_tmpl_obj.update_products_in_woo(instance, woo_templates,
                                                            self.update_image_in_product_update)
    ...
    def check_products(self, woo_templates):
        if self.env['woo.product.product.ept'].search(
                [('woo_template_id', 'in', woo_templates.ids), ('default_code', '=', False)]):
    ...
    def sync_products(self):
        ...
        for instance in self.instance_ids:
            if instance.woo_version == 'old':
                woo_template_obj.sync_products(instance, update_price=instance.sync_price_with_product,
                                               sync_images_with_product=instance.sync_images_with_product,
                                               skip_existing_products=skip_existing_products)
            elif instance.woo_version == 'new':
                woo_template_obj.sync_new_products(instance, update_price=instance.sync_price_with_product,
                                                   sync_images_with_product=instance.sync_images_with_product,
                                                   skip_existing_products=skip_existing_products)
        return True
    ...
    def import_stock(self):
        ...
        for instance in self.instance_ids:
            ...
            if instance.is_latest:
                products = self.env['woo.product.product.ept'].search(
                    [('variant_id', '!=', False), ('exported_in_woo', '=', True),
                     ('woo_instance_id', '=', instance.id)])
                for product in products:
                    if product.woo_template_id.woo_tmpl_id == product.variant_id:
                        try:
                            ...
                            if res.status_code not in ['201', '200']:
                                if res.json().get('manage_stock'):
                                    if product.product_id.type == 'product':
                                        stock_qty = res.json().get('stock_quantity')
                                        stock_data.update({'stock_qty': stock_qty})
                                        stock_data.update({'product_id': product.product_id.id})
                            ...
                            transaction_log_obj.create(
                                {'message': 'Import Stock for product %s not perfom.\n Error %s' % (product.name, e),
                                 'mismatch_details': True,
                                 'type': 'product',
                                 'woo_instance_id': instance.id
                                 })
                        ...
                        try:
                            res = wcapi.get(
                                "products/%s/variations/%s" % (product.woo_template_id.woo_tmpl_id, product.variant_id))
                            stock_data = {}
                            if res.status_code not in ['201', '200']:
                                if res.json().get('manage_stock'):
                                    if product.product_id.type == 'product':
                    ...
                    try:
                        ...
                        if res.status_code not in ['201', '200']:
                            ...
                            if response.get('type') == 'simple':
                                stock_data = {}
                                manage_stock = response.get(
                                    'managing_stock') if instance.woo_version == 'old' else response.get('manage_stock')
                                if manage_stock and product_template.product_tmpl_id.product_variant_ids.type == 'product':
                                    stock_qty = response.get('stock_quantity')
                                    stock_data.update({'stock_qty': stock_qty})
                                    stock_data.update(
                                        {'product_id': product_template.product_tmpl_id.product_variant_ids.id})
                                    prodcuts_stock.append(stock_data)
                            else:
                                    ...
                                    for product in product_template.product_tmpl_id.product_variant_ids:
                                        if variation.get('sku') == product.barcode and product.type == 'product':
                                            if variation.get('managing_stock') or variation.get('manage_stock'):
                                                ...
                                                stock_data.update({'stock_qty': stock_qty})
                                                stock_data.update({'product_id': product.id})
                                                prodcuts_stock.append(stock_data)
                        else:
                            transaction_log_obj.create({
                                'message': 'Import Stock for product %s has not proper response.\n Response %s' % (
            ...
            if prodcuts_stock:
                invetory_id = self.env['stock.inventory'].create({'name': 'Inventory For Instance %s' % (instance.name),
                ...
                for product_stock in prodcuts_stock:
                    stock_inventory_line = stock_inventory_line_obj.create(
                        {"inventory_id": invetory_id.id, "product_id": product_stock.get('product_id'),
                         "location_id": invetory_id.location_id.id, "product_qty": product_stock.get('stock_qty')})
